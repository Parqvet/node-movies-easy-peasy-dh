...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			let moviesAlfa = moviesTitles.sort().join('\n');

			res.write(`Lstado de peliculas: ${moviesAlfa}`);
		...
		case '/en-cartelera':
			...
			// let moviesTitlesCartelera = movies.map(movie => movie.title.toLocaleUpperCase());
			let moviesReviewCartelera = movies.map(movie => `${movie.title.toLocaleUpperCase()}: ${movie.overview}\n\n`);
			let moviesReviewSpace = moviesReviewCartelera.join('\n');
		...
		case '/mas-votadas':
			...
			// let masVotadasAlfa = masVotadas.sort();
			...
			masVotadas.forEach(movie => {
				promedio = promedio + movie.vote_average;
			})

			promedio = promedio/masVotadas.length;
			
			res.write(`Promedio de las mas votadas: ${promedio}`); */

			let votadasAvarage = masVotadas.map(movie => movie.vote_average); // me devuelve un nuevo array con el puntaje de cada una de las movies

			votadasReduce = votadasAvarage.reduce((totalMovies, movie) => totalMovies + movie);
			let promedio = votadasReduce / masVotadas.length;
			res.write(`Promedio: ${promedio}`);
